...
function LogoEasterEgg() {
  ...
  const PHRASES = [
    ...
    "иди работай бро 💀",
    "серьёзно? опять сюда?",
    "это не кнопка денег сорри",
    "ну и зачем ты это сделал",
    "окей стоп хватит",
    "тебе норм вообще?",
    "это уже третий раз",
    "ладно я не осуждаю",
    "go post something bro",
    "редит сам себя не постит",
    "модели ждут а ты тут",
    "дедлайн ближе чем кажется",
    "touch grass потом возвращайся",
    "один клик = один потерянный пост",
    "твой тим лид всё видит 👁",
    "статистика не растёт от этого",
    "закрой и иди работать",
    "ладно отдохнул? теперь постить",
    "стоп. открой чеклист",
    "реально? ок ок понял тебя",
    ...
    "сделай сейчас — потом скажешь спасибо",
    "тихий режим = денежный режим",
    "фокус и всё получится",
    "ещё немного и будет результат",
    "работаем пока другие спят",
    ...
    "терпение и труд всё перетрут (c)",
    "ты справишься — я верю",
    // Про сферу и бизнес
    "редит не спит и ты не спи",
    "трафик сам себя не нагонит",
    "подписчики ждут контента",
    "конкуренты уже постят а ты?",
    "rpm себя не поднимет",
    "ctr растёт от постов а не кликов сюда",
    "модели не ведут себя сами",
    "выручка любит активность",
    "кто постит тот и зарабатывает",
    "алгоритм любит регулярность",
    "топ суб сам себя не покорит",
    "новые подписчики ждут контента",
    "аудитория сама не придёт",
    "карма не фармится кнопками здесь",
    "посты = деньги. всё просто",
    "апвоуты не будут сами по себе",
    "суб не закрыт? значит постим",
    "каждый апвоут это потенциальный клиент",
    "трафик = деньги. иди постить",
    "пора добавить запись в чеклист",
    // Про inferyx
    "inferyx v3.0 говорит: работай",
    "это платформа для работы а не кликов",
    "inferyx создан чтобы ты зарабатывал",
    "инфериксу нравится когда ты работаешь",
    "нажми на задачи а не сюда",
    "этот логотип не генерирует доход",
    "inferyx > прокрастинация",
    "система работает. оператор нет",
    "инферикс смотрит на тебя 👁",
    "v3.0 и ты всё ещё здесь?",
    "логотип просто картинка. посты = деньги",
    "разработчик устал от твоих кликов",
    "inferyx говорит гоу постить",
    "ты нашёл пасхалку. молодец. иди работай",
    "это секрет но между нами: иди постить",
    // Абсурд и юмор
    "404: мотивация временно недоступна",
    "перезагрузи себя нажми f5",
    "error: слишком много кликов на логотип",
    "система в норме. пользователь нет",
    "загружаю мотивацию... 0%",
    "ты нашёл секретную кнопку. и что?",
    "это не читерский код",
    "unlock achievement: прокрастинатор",
    "нажми ещё — я не против честно",
    "хорошо хоть не тикток листаешь",
    "ладно признаю это прикольная кнопка",
    "я бы тоже нажимал если честно",
    "окей ты победил. теперь иди работать",
    "поздравляю ты нашёл кота 🐱",
    "рожки — это стиль не угроза",
    // Повседневные
    "как дела? у меня норм. теперь работай",
    "кстати как там чеклист?",
    "кофе выпил? ладно иди постить",
    "сегодня пятница но посты важнее",
    "это воскресенье? всё равно постим",
    "ночная смена — самая продуктивная",
    "выспался? молодец. теперь работать",
    "обед уже был? ладно продолжай",
    "телефон отложи. открой чеклист",
    "пять минут перерыва уже прошло",
    "ладно ладно ещё пять минут",
    "не забудь выйти на смену",
    "тим лид уже написал кстати",
    "проверь уведомления может там что-то",
    "чат не почитал? там важное",
    // С отсылками
    "winter arc активирован 🥶",
    "glow up начинается с постов",
    "no days off это не просто слова",
    "stay on your grind fr fr",
    "this is the way (мандалорец)",
    "lets get this bread 🍞",
    "we move только вперёд",
    "main character не прокрастинирует",
    "боковой персонаж листает логотип",
    "sigma не нажимает на логотип дважды",
    "rizz без постов — ничто",
    "gng не отдыхает в рабочее время",
    "npc нажимают сюда. ты не npc же?",
    "speedrun рабочего дня начался",
    "achievement unlocked: нашёл пасхалку",
    // Короткие удары
    "нет.",
    "стоп.",
    "работай.",
