...
export default function Layout({ children }) {
  ...
  const Avatar = ({ size = 32, fontSize = 13, radius = "50%" }) => (
    <div style={{
      ...
      fontWeight: 700, color: "#fff", flexShrink: 0, userSelect: "none", lineHeight: 1,
    }}>
      ...
      <motion.div animate={{ width: collapsed ? 70 : 240 }} transition={{ duration: 0.3, ease: "easeInOut" }}
        ...
        <div style={{ borderTop: `1px solid ${t.border}`, paddingTop: "12px", marginTop: "12px" }}>
          <button onClick={toggle} className="nav-link"
            style={{ width: "100%", background: "none", border: "none", cursor: "pointer", justifyContent: collapsed ? "center" : "flex-start", marginBottom: "2px" }}>
          ...
          <div onClick={() => navigate("/profile")}
            ...
            {!collapsed && (
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: "10px", fontWeight: 600, padding: "1px 6px", borderRadius: "20px", background: `${roleColor}20`, color: roleColor, letterSpacing: "0.5px", textTransform: "uppercase", display: "inline-block" }}>
                  {roleLabel}
                </div>
              </div>
            )}
        ...
        {mobileOpen && (
          <>
            ...
            <motion.div initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} transition={{ type: "spring", damping: 25 }}
              ...
              <button onClick={toggle} className="nav-link" style={{ background: "none", border: "none", cursor: "pointer", marginBottom: "2px" }}>
