...
import { Plus, Trash2, Settings as SettingsIcon, Globe, User, Moon, Sun } from "lucide-react";
...
export default function Settings() {
  ...
  const deletePlatform = async (id) => { if (!isAdmin) return; await deleteDoc(doc(db, "platforms", id)); };
  const deleteModel = async (id) => { if (!isAdmin) return; await deleteDoc(doc(db, "models", id)); };
      ...
      <div style={{ display: "flex", gap: "4px", background: t.bgCard, borderRadius: "12px", padding: "4px", marginBottom: "28px", width: "fit-content", border: `1px solid ${t.border}` }}>
        {tabs.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setActiveTab(id)}
            style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 18px", borderRadius: "8px", border: "none", cursor: "pointer", fontSize: "13px", fontWeight: 600, transition: "all 0.2s",
              background: activeTab === id ? "rgba(124,58,237,0.2)" : "transparent",
              color: activeTab === id ? "#a78bfa" : t.textMuted }}>
            <Icon size={15} />{label}
          </button>
        ))}
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: "16px", padding: "24px", marginBottom: "20px" }}>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <input placeholder="Название платформы..." value={newPlatform} onChange={e => setNewPlatform(e.target.value)}
                onKeyDown={e => e.key === "Enter" && addPlatform()} style={inputStyle} />
      ...
      {activeTab === "models" && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: "16px", padding: "24px", marginBottom: "20px" }}>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <input placeholder="Имя модели..." value={newModel} onChange={e => setNewModel(e.target.value)}
                onKeyDown={e => e.key === "Enter" && addModel()} style={inputStyle} />
                ...
                {models.map((m, i) => (
                  <motion.div key={m.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      <div style={{ width: "34px", height: "34px", borderRadius: "8px", background: "linear-gradient(135deg, #7c3aed, #db2777)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: 700, color: "#fff" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: "16px", padding: "24px" }}>
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              {[{ m: "dark", label: "Тёмная", icon: Moon, desc: "Тёмный фон, светлый текст" }, { m: "light", label: "Светлая", icon: Sun, desc: "Светлый фон, тёмный текст" }].map(({ m: themeMode, label, icon: Icon, desc }) => (
                <button key={themeMode} onClick={() => themeMode !== mode && toggle()}
                  style={{ flex: 1, padding: "20px", borderRadius: "14px", border: `2px solid ${mode === themeMode ? "#7c3aed" : t.border}`, background: mode === themeMode ? "rgba(124,58,237,0.1)" : t.bgCardHover, cursor: "pointer", textAlign: "center", transition: "all 0.2s" }}>
                  ...
                  {mode === themeMode && (
                    <div style={{ marginTop: "10px", background: "rgba(124,58,237,0.2)", color: "#a78bfa", fontSize: "11px", fontWeight: 700, padding: "3px 10px", borderRadius: "20px", display: "inline-block" }}>
                      Активна
                    </div>
                  )}
                </button>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: "16px", padding: "24px", marginTop: "20px" }}>
            ...
            <p style={{ color: t.textMuted, fontSize: "14px", marginBottom: "16px", lineHeight: "1.6" }}>
              Если вы столкнулись с техническими проблемами или есть предложения — наша команда готова помочь.
            </p>
