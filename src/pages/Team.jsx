...
export default function Team() {
  ...
  useEffect(() => {
    const unsubs = [
      onSnapshot(collection(db, "users"), snap => { setUsers(snap.docs.map(d => ({ id: d.id, ...d.data() }))); setLoading(false); }),
  ...
  const createMember = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "users"), { uid: cred.user.uid, name: form.name, email: form.email, role: form.role, createdAt: new Date().toISOString() });
          ...
          {grouped.map(({ role, members }) => (
            <div key={role}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "12px" }}>
                ...
                <span style={{ color: ROLE_COLORS[role], fontSize: "13px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "1px" }}>{ROLE_LABELS[role]}</span>
                  ...
                  return (
                    <motion.div key={member.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                      ...
                      <div style={{ display: "flex", gap: "8px" }}>
                        {[{ v: stats.total, l: "записей", c: t.text }, { v: stats.traffic.toLocaleString(), l: "трафик", c: "#10b981" }, { v: stats.today, l: "сегодня", c: "#0ea5e9" }].map((s, i) => (
